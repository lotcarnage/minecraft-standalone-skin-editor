...
色の編集は、カラー入力 UI（input type="color"）で色の選択が行われた時点で反映してください。

カラー入力の値が変更された場合、編集対象となっているパレット index に対して、選択された色を即座に反映します。
この操作に追加の「反映」操作は必要ありません。

パレット index 0（透明色）は編集対象外です。
