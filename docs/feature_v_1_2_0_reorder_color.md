...
ユーザーがドラッグ元スウォッチをドラッグ先スウォッチ上で離した場合、両スウォッチの RGB 値を入れ替えます。スウォッチ配列のインデックス順は物理的に変化させず、各スロットに格納されている RGBA データを交換することで視覚的な並び順を変更します。さらに、編集キャンバスが保持しているパレットインデックスも全レイヤーで入れ替え、ドラッグ完了直後からキャンバス上の各画素が新しい並び順に追従するようにします。入れ替え後はパレット表示を再描画し、現在選択中の色が交換後も正しく示されるように選択状態を再評価します。
...
4. `pixelIndexByLayer` 配列を全て走査し、`sourceIndex` を参照している画素を `targetIndex` に、`targetIndex` を参照している画素を `sourceIndex` に入れ替える。

処理は同期的に完了させ、交換後にパレット表示、カラーピッカー、埋め込みモジュールへ新しい配列参照を通知します。選択中インデックスおよび編集中インデックスは、交換前に参照していた色へ追従できるよう、それぞれ `sourceIndex` と `targetIndex` を相互に置き換えて更新します。
